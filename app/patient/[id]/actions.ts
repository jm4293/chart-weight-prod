...
export async function registerWeight(id: number, weight: string) {
  ...
  const ret = await api.post("http://localhost:5007/patient/weight", {
    json: { id, weight },
    headers: {
...
export async function addWeightWithImage(id: number, file: File) {
  ...
  const ret = await api.post("http://localhost:5007/patient/weight-image", {
    body: formData,
    headers: {
      Cookie: `${cookie?.name}=${cookie?.value}`,
    },
...
export async function deleteWeight(id: number) {
  ...
  const ret = await api.delete(`http://localhost:5007/patient/weight/${id}`, {
    headers: {
      "Content-Type": "application/json",
      Cookie: `${cookie?.name}=${cookie?.value}`,
    },
